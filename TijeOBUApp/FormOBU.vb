...
Public Class FormOBU
    Private lastProcessedTime As DateTime = DateTime.MinValue
    Private processInterval As TimeSpan = TimeSpan.FromSeconds(1)
    Private lines() As String
    Private gpsDataQueue As New Queue(Of String)
    Private processingTimer As System.Windows.Forms.Timer
    ...
    Private routeFormInstance As frmRute = Nothing


    Private Sub InitializeProcessing()
        processingTimer = New System.Windows.Forms.Timer()
        processingTimer.Interval = 1000 ' 1000ms = 1 second
        AddHandler processingTimer.Tick, AddressOf ProcessGpsDataPacket
        processingTimer.Enabled = True
    End Sub

    Private Sub ProcessGpsDataPacket(ByVal sender As Object, ByVal e As EventArgs)
        ' Lock the queue to avoid concurrent modifications
        SyncLock gpsDataQueue
            ' Process one data packet from the queue (if available)
            If gpsDataQueue.Count > 0 Then
                Dim data As String = gpsDataQueue.Dequeue()
                ProcessGPSData(data)
            End If
        End SyncLock
    End Sub

    ' This function is called when new GPS data is received
    Private Sub QueueGpsData(ByVal data As String)
        ' Lock the queue before adding new data to avoid concurrency issues
        SyncLock gpsDataQueue
    ...
    Private Sub ProcessGPSData(ByVal data As String)
        ...
        Try
            ...
            ' Only process GPS data if more than 1 second has passed since the last processing
            If currentTime - lastProcessedTime > processInterval Then
...
2:
    ...
    Private Sub btnPilihRute_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPilihRute.Click
        ...
        If routeFormInstance Is Nothing OrElse routeFormInstance.IsDisposed Then
            routeFormInstance = New frmRute()
            routeFormInstance.Show()
        Else
            routeFormInstance.BringToFront()
            routeFormInstance.Focus()
        End If
    ...
    Private Sub SendMqttData()
        Dim publishSuccessful As Boolean = False
        Dim latitude As Double = lastLatitude
        Dim longitude As Double = lastLongitude
        Dim speed As Integer = CInt(Math.Ceiling(lastSpeed))
        Dim bearing As Integer = CInt(Math.Ceiling(lastBearing))
        Dim totalDistance As Integer = CInt(Math.Ceiling(lastTotalDistance))
        Dim dateValue As DateTime = DateTime.Now ' Or any specific DateTime value
        Dim epoch As DateTime = New DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)
        Dim timeStamp As Long = CLng((dateValue.ToUniversalTime() - epoch).TotalMilliseconds)

        ' Convert the data to JSON format
        Dim jsonData As String = CreateJsonPayload(busBodyNo, busDeviceId, latitude, longitude, speed, bearing, totalDistance, CLng(timeStamp))

        Try
            mqttClient.Publish("/tracking/obu", System.Text.Encoding.UTF8.GetBytes(jsonData), MqttMsgBase.QOS_LEVEL_AT_LEAST_ONCE, False)
            ' write data into textfile
            writeDataLog(latitude, longitude)
        ...
        Catch ex As Exception
            lblMqttMsg.Text = "Failed to send data to broker"
        End Try
    End Sub


    Private Function CreateJsonPayload(ByVal busBodyNo As String, ByVal busDeviceId As String, ByVal latitude As Double, ByVal longitude As Double, ByVal speed As Integer, ByVal bearing As Integer, ByVal totalDistance As Integer, ByVal timeStamp As Long) As String
        Dim jsonFormat As String = "{{ ""busBodyNo"": ""{0}"", ""busDeviceId"" : ""{1}"", ""latitude"" : {2}, ""longitude"" : {3}, ""speed"" : {4}, ""bearing"": {5}, ""totalDistance"" : {6}, ""deviceTimestamp"" : {7} }}"

        Return String.Format(jsonFormat, busBodyNo, busDeviceId, latitude.ToString("F7", System.Globalization.CultureInfo.InvariantCulture), longitude.ToString("F7", System.Globalization.CultureInfo.InvariantCulture), speed, bearing, totalDistance, timeStamp)
    End Function
    ...
    Private Sub writeDataLog(ByVal isLat As Double, ByVal isLong As Double)
        Dim currentTime As String = DateTime.Now.ToString("HH:mm:ss")
        Dim data As String = "lat: " & isLat.ToString() & ", long: " & isLong.ToString() & ", time: " & currentTime

        ' Specify the file path
        Dim filePath As String = "\Flash Disk\\obu4g config\gps_data_log.txt"

        ' Append the data to the file
        Try
            Using writer As New StreamWriter(filePath, True) ' True for append mode
                writer.WriteLine(data)
            End Using
            UpdateTextBox("Data written to file: " & data)
        Catch ex As Exception
            UpdateTextBox("Error writing to file: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub ButtonWelcome_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonWelcome.Click
        ...
        If ButtonWelcome.BackColor = Color.Black Then
            ...
            Try
                ...
                'FIXME irul
                Me.sendRunningText(String.Concat(New String() {busDeviceId, "/", REGNO, "/", VECCODE}), 0, 0)
                Me.sendRunningText(String.Concat(New String() {busDeviceId, "/", REGNO, "/", VECCODE}), 1, 0)
                Me.sendRunningText(String.Concat(New String() {busDeviceId, "/", REGNO, "/", VECCODE}), 2, 0)
